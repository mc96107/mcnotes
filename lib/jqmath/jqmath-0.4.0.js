...
var jqMath = function() {
	...
	M.newMe = function(tagName, argsP /* for appendMeArgs() */, attrsP, docP) {
		...
		if (tagName == 'fmath' || tagName == 'mn' || tagName == 'mtext' || tagName == 'mstyle'
		|| tagName == 'merror' || tagName == 'mpadded' || tagName == 'mphantom'
		|| tagName == 'menclose' || tagName == 'mprescripts' || tagName == 'none')
			;
		else if (tagName == 'mi') {
	...
	function ovScan() /* overbar */ {
		return [M.newMe('menclose', scanMeTok('\\ov')[0], { notation: 'top' }, docP_), null];
	}
	...
	M.macros_ /* each returns a tokP */ = {
		...
		ds: F(mvScan, 'double-struck'), bf: F(mvScan, 'bold-fraktur'), ov: ovScan,
		minsize: minsizeScan, mrowOne: mrow1Scan, binom: binomScan
	};
